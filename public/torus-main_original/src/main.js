...
function getNoiseMode(typeName, enabled) {
    if (!enabled) return 0;
...
let manifestClock = new THREE.Clock(false);
...
function rebuildRover() {
    ...
    // Helper: create a line with all required attributes
    function addLine(positions, uArr, vArr, isVertArr, isExtraArr) {
        ...
        geo.setAttribute('aIsThroatExtra', new THREE.BufferAttribute(isExtraArr, 1));
        roverGroup.add(new THREE.Line(geo, fillMaterial));
    }

    // 1. Poloidal (Vertical) Lines
    const pCount = Math.max(1, Math.floor(params.unifiedPoloidalCount));
    const pSegs = Math.max(2, Math.floor(params.unifiedPoloidalSegments));
    ...
    for (let i = 0; i < pCount; i++) {
        ...
        const aIsThroatExtra = new Float32Array(n);
        ...
        for (let j = 0; j <= pSegs; j++) {
            ...
            aU[j] = u; aV[j] = v; aIsVertical[j] = 1.0; aIsThroatExtra[j] = 0.0;
        }
        addLine(pos, aU, aV, aIsVertical, aIsThroatExtra);
    }

    // 2. Toroidal (Horizontal) Rings
    const tCount = Math.max(1, Math.floor(params.unifiedToroidalCount));
    const tSegs = Math.max(2, Math.floor(params.unifiedToroidalSegments));
    ...
    for (let i = 0; i < tCount; i++) {
        ...
        for (let j = 0; j <= tSegs; j++) {
            ...
            pos[j * 3 + 1] = r * Math.sin(phi);
            pos[j * 3 + 2] = (R + r * Math.cos(phi)) * Math.sin(theta);
            aU[j] = u; aV[j] = v; aIsVertical[j] = 0.0; aIsThroatExtra[j] = 0.0;
        }
        addLine(pos, aU, aV, aIsVertical, aIsThroatExtra);
    }

    // 3. Extra Throat Lines (generated over full range, masked by shader)
    if (params.throatExtraEnabled) {
        const eCount = Math.max(1, Math.floor(params.throatExtraLineCount));
        const eSegs = Math.max(2, Math.floor(params.throatExtraSegments));
        const geomType = params.throatExtraGeomType;

        if (geomType === 'Vertical Lines' || geomType === 'Both') {
            // Extra vertical lines spanning full v range (shader handles masking)
            for (let i = 0; i < eCount; i++) {
                ...
                for (let j = 0; j <= eSegs; j++) {
                    ...
                    pos[j * 3 + 1] = r * Math.sin(phi);
                    pos[j * 3 + 2] = (R + r * Math.cos(phi)) * Math.sin(theta);
                    aU[j] = u; aV[j] = v; aIsVertical[j] = 1.0; aIsThroatExtra[j] = 1.0;
                }
                addLine(pos, aU, aV, aIsVertical, aIsThroatExtra);
            }
        }
        ...
        if (geomType === 'Horizontal Rings' || geomType === 'Both') {
            // Extra horizontal rings spread across full v range (shader handles masking)
            for (let i = 0; i < eCount; i++) {
                const v = i / eCount;
                const n = eSegs + 1;
                const pos = new Float32Array(n * 3);
                ...
                for (let j = 0; j <= eSegs; j++) {
                    ...
                    pos[j * 3 + 1] = r * Math.sin(phi);
                    pos[j * 3 + 2] = (R + r * Math.cos(phi)) * Math.sin(theta);
                    aU[j] = u; aV[j] = v; aIsVertical[j] = 0.0; aIsThroatExtra[j] = 1.0;
                }
                addLine(pos, aU, aV, aIsVertical, aIsThroatExtra);
            }
        }
...
function buildLoopsFromEdges(edgesGeo) {
...
function animate() {
    ...
    if (fpsDisplayVisible) {
        ...
        if (now - lastFpsTime >= 1000) {
            perfFps.innerText = frameCount;
            perfMs.innerText = (1000 / frameCount).toFixed(1) + ' ms';
            frameCount = 0;
            lastFpsTime = now;
        }
    }
    ...
    if (fpsDisplayVisible && frameCount === 0) { // Update detailed stats only once per second to avoid DOM spam
        perfCalls.innerText = renderer.info.render.calls;
        perfTriangles.innerText = renderer.info.render.triangles.toLocaleString();
        perfGeometries.innerText = renderer.info.memory.geometries;
        perfTextures.innerText = renderer.info.memory.textures;
    }
}
...
function initGui() {
    ...
    colorsFolder.addColor(params, 'lineColor').name('Line Color (Base)').onChange(val => uniforms.uLineColor.value.set(val));
    colorsFolder.addColor(params, 'glowColor').name('Glow Color (Light)').onChange(val => uniforms.uGlowColor.value.set(val));
    colorsFolder.addColor(params, 'glowColor2').name('Glow Color 2 (Gradient)').onChange(val => uniforms.uGlowColor2.value.set(val));
    colorsFolder.addColor(params, 'cursorLightColor').name('Cursor Color').onChange(val => uniforms.uCursorColor.value.set(val));

    const cameraFolder = gui.addFolder('Camera & Rotation');
    cameraFolder.add(params, 'cameraZ', 1, 30).name('Camera Z').onChange(val => { camera.position.z = val; });
